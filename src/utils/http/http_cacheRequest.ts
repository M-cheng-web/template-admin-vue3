...
export default function CacheRequest(this: any) {
  ...
  this.CACHES = new Proxy(
    {},
    {
      get: function (target: obj, key: string) {
        let value = target[key]
        console.log(`${key} 被读取`, value)
        if (!value) value = _this.getCacheItem(key)
        return value
      },
      set: function (target: obj, key: string, value) {
 ...
 * 没有则返回null
 */
CacheRequest.prototype.getCacheItem = function (key: string) {
  const cache = window.localStorage.getItem(cacheStorageKey) || ''
 ...
 * 根据请求Key去存储数据
 */
CacheRequest.prototype.setCacheItem = function (key: string, value: object) {
  const cache = window.localStorage.getItem(cacheStorageKey) || ''
 ...
 * 缓存 - 请求拦截
 */
CacheRequest.prototype.requestInterceptor = function (config: any, axios: any) {
  if (config.cache) {
 ...
 * 缓存 - 响应拦截
 */
CacheRequest.prototype.responseInterceptor = function (response: any) {
  if (
    response &&
